...
faq:
  - q: 'What should I do right after registering, before the website?'
    a: 'Buy the domain in the company''s name (not the contractor''s), set up email on your own domain and create a Google Business Profile. All three are free or cost a few euros and fit into one evening. The website can come after.'
...
## Week four: measurement, without which nothing can be decided

A website without enquiry tracking is an expense you will never know paid off.

The minimum from day one: Google Analytics 4 through Tag Manager, an event on the **successful** form submission (not on the button click), marked as a key event, linked to Search Console. It sounds technical, but it is the only way to know a month later where the enquiries came from and what they cost. The steps: [configuring GA4](/en/blog/ga4-setup/).

Search Console belongs here too: free, and it shows which queries the site appears for. For a new company it is the only source of truth about what people are searching.

## What can wait

Plainly, because it saves the first month's money:

- **Paid advertising.** Until the site has measurement and a page for a specific service, advertising is a paid exit for visitors. First the points above, then [what advertising costs](/en/blog/google-ads-cost-slovakia/).
- **Social media as the main channel.** For B2B services it brings little; for a salon or a restaurant yes, but after the Google profile.
- **A logo for thousands of euros.** The customer buys a solution to a problem, not a logo.
- **A chatbot, a CRM, automations.** With five enquiries a week there is nothing to automate.
...
1. The `.sk` domain registered to the company, auto-renewal on.
2. Email on the domain + SPF, DKIM, DMARC.
3. A Google Business Profile, the same NAP as everywhere else.
...
Details on the [web development service page](/en/service/webdev/). Related: [opening an s.r.o.](/en/blog/open-sro-in-slovakia/), [živnosť or s.r.o.](/en/blog/zivnost-or-sro/) and [accepting card payments](/en/blog/accept-card-payments/).
